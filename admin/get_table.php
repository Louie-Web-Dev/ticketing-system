...
FROM (
    SELECT con_date, category, sub_cat, 1 AS pending_count, 0 AS solved_count
    FROM concerns
    WHERE status = 'pending'" .
    (!empty($whereConditions) ? ' AND ' . implode(' AND ', $whereConditions) : '') . "
    UNION ALL
    SELECT con_date, category, sub_cat, 0 AS pending_count, 1 AS solved_count
    FROM concerns
    WHERE status = 'done'" .
    (!empty($whereConditions) ? ' AND ' . implode(' AND ', $whereConditions) : '') . "
) AS sub
...
$data = array_fill(1, 12, ['pending' => 0, 'solved' => 0]);
...
echo '<table><thead><tr><th>Month</th><th>Pending</th><th>Solved</th></tr></thead><tbody>';
foreach ($months as $num => $name) {
    $pending = $data[$num]['pending'];
    $solved = $data[$num]['solved'];
    $total_pending += $pending;
    $total_solved += $solved;
    echo "<tr><td>" . htmlspecialchars($name) . "</td><td>$pending</td><td>$solved</td></tr>";
}
echo "<tr style='font-weight:bold'><td>Total</td><td>$total_pending</td><td>$total_solved</td></tr>";
echo '</tbody></table>';
